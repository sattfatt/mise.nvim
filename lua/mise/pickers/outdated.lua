...
function M.pick(opts)
  ...
  local function finder(_opts, _ctx)
    return function(cb)
      local stdout, _, code = util.run({ "outdated", "--json" }, { cwd = cwd })
      if code ~= 0 then
      ...
      -- Empty object means everything is up to date
      ...
      for _, name in ipairs(names) do
        ...
        local bump = info.bump -- "major", "minor", "patch", or nil
  ...
  local actions = {
    ...
    -- Upgrade ALL outdated tools with confirmation
