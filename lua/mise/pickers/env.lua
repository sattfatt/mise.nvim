...
function M.pick(opts)
  ...
  local function finder(_opts, _ctx)
    return function(cb)
      local stdout, _, code = util.run({ "env", "-J" }, { cwd = cwd })
      if code ~= 0 then
      ...
      -- Sort keys for stable, predictable ordering
      ...
      for _, key in ipairs(keys) do
        ...
        -- Special handling for PATH: split on : for readable preview
        local preview_lines
        if key == "PATH" then
  ...
  local function format(item, _picker)
    ...
    -- Value (truncated for PATH and long values)
    local display_val = item.env_value
    if #display_val > 80 then
      display_val = display_val:sub(1, 77) .. "..."
    end

    -- PATH gets special coloring
    local val_hl = item.env_key == "PATH" and "SnacksPickerDir" or "Normal"
    ret[#ret + 1] = { display_val, val_hl }

    return ret
  end

  local actions = {
    -- Yank the value to clipboard (default <CR>)
    ...
    -- Open nearest mise.toml to edit the [env] section
    edit_env_config = function(picker, item)
      ...
      if found[1] then
        ...
        -- Jump to [env] section
        vim.fn.search("\\[env\\]", "w")
        -- Then search for the specific key
        vim.fn.search(vim.fn.escape(item.env_key, "[]().*+?^${}|\\"), "w")
      else
        util.notify("No mise.toml found in current directory or parents", vim.log.levels.WARN)
      end
    end,
  }
